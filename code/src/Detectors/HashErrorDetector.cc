...
bool HashErrorDetector::test(messageInfo message,
 ...
 * @param params Simulation parameters.
 * @return true if the error detector concludes that the message can be causally delivered and false otherwise. */
bool HashErrorDetector::hashPartialDependencies(const messageInfo& message,
                                                const vector<messageInfo>& delivered,
                                                const vector<unsigned int>& incrementedClockEntries,
    ...
    vector<idMsg> possibleDependencies = sortPossibleDependenciesSet(
        message, createPossibleDependenciesSet(message, delivered, incrementedClockEntries, controller));
    ...
    nbHashs++;
    return hashIsEqual(message.hash, hashPartialDependencies({}),
                       nbHashs);  // tries to compute the hash with no dependencies
}
 ...
 * @param params Simulation parameters.
 * @return true if the error detector concludes that the message can be causally delivered and false otherwise. */
bool HashErrorDetector::hashTotalDependencies(const messageInfo& message,
                                              const vector<messageInfo>& delivered,
                                              const vector<unsigned int>& incrementedClockEntries,
                                              const TotalDependencies& processDependencies,
                                              Controller* controller,
    ...
    TotalDependencies baseDependencies =
        createBaseDependencies(message, delivered, incrementedClockEntries, processDependencies);
 ...
 * */
TotalDependencies HashErrorDetector::createBaseDependencies(const messageInfo& message,
                                                            const vector<messageInfo>& delivered,
                                                            const vector<unsigned int>& incrementedClockEntries,
                                                            const TotalDependencies& processDependencies) {
    //    for (unsigned int entry : incrementedClockEntries) commented because interfers with hash computed with clock
    //    difference (+small impact)
    ...
    for (const messageInfo& possibleDep : delivered) {
        if (!isPossibleDependency(message, possibleDep) ||
            !isConsideredAsDependency(message, possibleDep))  // message cannot be a dependency or received to recently
                                                              // (in which case it is considered as possible dependency)
 ...
 * */
vector<messageInfo> HashErrorDetector::createPossibleDependenciesSet(
...
/** Increments the vector nbMsgWronglyConsideredCausalDep while avoiding out of array bounds increments. Raises an error
 * in case of out of array bounds increment.
...
/** Increments the vector nbMsgToCombine while avoiding out of array bounds increments. Raises an error in case of out
 * of array bounds increment.
 * @param entry Entry to increment.*/
...
AppMsg* HashErrorDetector::prepareMessage(AppMsg* m,
                                          const vector<messageInfo>& delivered,
