...
typedef struct s_stats_hashErrorDetector {
    ...
    /** Number of messages the node wrongly considered as being causal dependencies in the hash computation of messages
     * it delivered (obtained by calling the controler). entry i : Number of times where in the hash computation to
     * deliver a message $m$ the node wrongly considered i messages as being causal dependencies of $m$. */
    vector<unsigned int> nbMsgWronglyConsideredCausalDep;
    /** Number of possible dependencies in the hash computation of messages.
     * entry i : Number of times where the node considered i messages as possible dependencies in the hash computation
     * of a message m. */
    vector<unsigned int> nbMsgToCombine;
    /** Number of done combinations to find the hash of delivere messages.
     * entry i : Number of times where the node computed i hashes to find a dependency set whose hash was equal to the
     * one of the message to deliver.*/
    vector<unsigned int> doneCombinationsToFindHash;
    /** Number of operations to compute a hash.
     * entry i : Number of times where the node did i operations to compute the hash of the causal dependencies of a
     * message. */
...
class HashErrorDetector : public ErrorDetector {
   public:
    ...
    bool hashPartialDependencies(const messageInfo& message,
                                 const vector<messageInfo>& delivered,
                                 const vector<unsigned int>& incrementedClockEntries,
                                 Controller* controller,
                                 SimulationParameters* params);
    bool hashTotalDependencies(const messageInfo& message,
                               const vector<messageInfo>& delivered,
                               const vector<unsigned int>& incrementedClockEntries,
                               const TotalDependencies& processDependencies,
                               Controller* controller,
    ...
    TotalDependencies createBaseDependencies(const messageInfo& message,
                                             const vector<messageInfo>& delivered,
                                             const vector<unsigned int>& incrementedClockEntries,
                                             const TotalDependencies& processDependencies);
    bool isPossibleDependency(const messageInfo& message, const messageInfo& possibleDep);
    vector<messageInfo> createPossibleDependenciesSet(const messageInfo& message,
                                                      const vector<messageInfo>& messageToChooseFrom,
                                                      const vector<unsigned int>& incrementedClockEntries,
                                                      Controller* controller);
    ...
    virtual vector<idMsg> sortPossibleDependenciesSet(const messageInfo& message,
                                                      const vector<messageInfo>& baseCombineSet) = 0;
    virtual PartialDependencies getPartialDependencies(const vector<messageInfo>& delivered,
                                                       const ProbabilisticClock& clock) = 0;
    virtual vector<idMsg> determineAndGetAppendedDependencies(const vector<messageInfo>& delivered,
                                                              const ProbabilisticClock& clock) = 0;

   private:
    /**Used to compute hashes.*/
    hash<string> hasher;
    /**Used to detect collisions when hashing dependencies sets, ie detect when different dependencies sets have the
     * same hash. Is inactivated by default because it uses a lot of memory space. To activate, set COLLISION_CONTROL in
     * simulatioParameters to true.*/
