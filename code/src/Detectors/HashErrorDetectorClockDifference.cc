...
vector<idMsg> ErrorDetectorClockDifference::determineAndGetAppendedDependencies(const vector<messageInfo>& delivered,
...
bool ErrorDetectorClockDifference::isConsideredAsDependency(const messageInfo& message,
                                                            const messageInfo& possibleDep) {
...
bool ErrorDetectorClockDifference::isConsideredAsPossibleDependency(const messageInfo& message,
                                                                    const messageInfo& possibleDep) {
    return message.clock.clockEntryDifference(possibleDep.clock) < clockDifferenceConsideredDependency;
}
 ...
 * */
vector<idMsg> ErrorDetectorClockDifference::sortPossibleDependenciesSet(const messageInfo& message,
                                                                        const vector<messageInfo>& baseCombineSet) {
...
/** Sets the clock difference over which a message is considered as having been delivered by all nodes and is therefore
 * considered as being a causal dependency of the message to deliver. See paper to get precision about the used formula.
...
void ErrorDetectorClockDifference::setClockDifferenceConsideredDependency(unsigned int messageLoad,
                                                                          unsigned nbIncrementedEntries) {
    clockDifferenceConsideredDependency =
        (messageLoad * nbIncrementedEntries * 0.15 + ASSUMED_CONCURRENT_MESSAGES * nbIncrementedEntries) * 1;
 ...
 * */
PartialDependencies ErrorDetectorClockDifference::getPartialDependencies(const vector<messageInfo>& delivered,
                                                                         const ProbabilisticClock& clock) {
    messageInfo message;
    message.clock = clock;
