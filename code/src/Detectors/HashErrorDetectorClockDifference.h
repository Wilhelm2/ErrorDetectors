...
/** Hash-based error detector that uses the clock difference between message to determine the causal dependencies of
 * messages. Assumes that probabilistic clocks increase over time. Thus, the greater the clock difference between two
 * messages, the greater the duration between their broadcast. Therefore, messages over a given clock difference are
 * considered as having been delivered by all nodes, and are not considered in the hash computation. */
class ErrorDetectorClockDifference : public HashErrorDetector {
   ...
   private:
    /** The clock difference over which a message is considered as having been delivered by all nodes and is thus not
     * taken into account in the hash computation.*/
    unsigned int clockDifferenceConsideredDependency = 100;
    /** The clock difference under which a message is considered as not being a dependency of a message to deliver,
     * because its low clock difference indicates that it has been broadcasted in a time interval very clock to the
     * broadcast of the message to deliver.*/
