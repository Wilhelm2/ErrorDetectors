...
void NodeDetector::initialize() {
    ...
    if (params->DeliveryController == SimulationParameters::Delivery::Mostefaoui)
        ...
        ((ErrorDetectorClockDifference*)detector)
            ->setClockDifferenceConsideredDependency(params->nbNodes / params->delaySend,
                                                     params->entriesIncrementedByProcess[0].size());
    } else
        detector = new ErrorDetectorInterval();
}

/** Instantiates an application message. Calls the error detector to append on the message the information the error
 * detector of the destination node will require to analyze the message.
...
AppMsg* NodeDetector::prepareBroadcast() {
    ...
    if (APPENDDEPENDENCIES) {
        ...
        stats.controlDataSize +=
            clock.size() * sizeof(unsigned int) + m->getDependencies().size() * 2 * sizeof(unsigned int);
    }
    return m;
}
