 ...
 * All nodes are connected to the communicationDispatcher and send messages to each other through the
 communicationDispatcher.
 * Using this star formation graph instead of a complete graph reduces the number of communication channels from N^2
 channels to N channels, which particularly reduces the simulation's initialization time.
 *
 * The communicationDispatcher routes messages to the destination, handles the broadcast of application messages, and
 sets the propagation delays of messages.
*/
...
class CommunicationDispatcher : public cSimpleModule {
   private:
    ...
    std::normal_distribution<double> distributionChannelDelayPair =
        std::normal_distribution<double>(CHANNELDELAY, 30000.);
