 ...
 *  The class reads the simulation parameters at the beginning of the simulation and other classes get access to those
...
class SimulationParameters : public cSimpleModule {
   ...
   public:
     ...
     * entriesIncrementedByProcess[0] contains the entries p_0 increments when broadcasting a message.*/
    vector<vector<unsigned int>> entriesIncrementedByProcess;
    /** Vector containing the dependencies to set in the hash computation of hash-based error detectors following the
     * number of messages that are considered of might being causal dependencies of the message to deliver*/
...
/**@def Determines if the collision controller is activated. It controls if two different dependencies sets have the
 * same hash. It is set to false by default because collision control uses a lot of memory.*/
#define COLLISION_CONTROL false
    ...
    enum class Delivery {
        ...
        ClockComparison,
        /** Uses Probabilistic clocks to causally order messages and executes the error detector of Mostéfaoui on a
           message $m$'s clock before delivering $m$*/
        Mostefaoui,
        /** Uses Probabilistic clocks to causally order messages and executes the hash-based error detector with the
           clock difference technique on a message $m$'s clock before delivering $m$*/
        HashClockDifference,
        /** Uses Probabilistic clocks to causally order messages and executes the hash-based error detector with the
           interval technique on a message $m$'s clock before delivering $m$*/
        HashIntervalDifference,
        /** Uses Probabilistic clocks to causally order messages and executes the hash-based error detector with the
           clock difference technique on a message $m$'s clock before delivering $m$, and recovers the dependencies of
           $m$ if the error detector concludes that $m$ might not be causally ordered*/
        Recovery,
        /** Uses Probabilistic clocks to causally order messages and executes the hash-based error detector with the
           clock difference technique on a message $m$'s clock before delivering $m$, executes the recoveryTest if the
           error detector concludes that $m$ might not be causally ordered, and recovers the dependencies of $m$ if the
           recovery test does not succeed in finding a dependencies set for $m$.*/
        RecoveryTest
    };
    /** Determines the type of the simulation's delivery control.*/
    static Delivery DeliveryController;
    /** Defines the type of dependencies used by hash-based error detectors to compute the hash of broadcasted
     * messages.*/
    enum class Dependencies {
        /** A node computes the hash of messages it broadcasts with the full dependency vector of the message to
           broadcast, ie with a dependency vector containing one entry per process in the system.*/
        Total,
        /** A node computes the hash of messages it broadcasts with only a subset of dependencies of the message to
           broadcast.*/
        Partial
    };
...
#define LIMIT_HASHS \
    200  // ATTENTION DOIT ÊTRE LE MÊME POUR LES MSG DANS COMPLETEHASHTEST ET RECOVERYTEST SINON DOIT
