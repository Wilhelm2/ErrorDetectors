...
/** @struct s_id
 *  @brief Each message has a unique identifier composed of the id of the node $p_i$ that sent it and the sequence
 * number $p_i$ associated to the message
 */
typedef struct s_id {
 ...
 *  @brief Structure containing information about received messages required to causally order them as well as to
 * execute the error detectors on them
 */
typedef struct s_messageInfo {
    ...
    /** The message's  hash */
    size_t hash;
    /** The message's causal dependencies appended. This field only contains dependencencies when APPENDDEPENDENCIES
     * (see simulationParameters.h) is defined as true*/
